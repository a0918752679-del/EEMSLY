...
- 原「套用驗證碼」改為登入頁：進入系統前必須先輸入內部驗證碼。
- 登入成功後才載入案件清單、Google Sheet 同步、匯出與行政追蹤功能。
- 保留原有案件審核、修改、刪除、行政追蹤、Google Sheet 同步與 8080 部署架構。

## 已內建功能

- 告發、通檢兩類案件分流管理。
- 匯出 Google Sheet 同格式資料；前台不提供 Excel 手動匯入，資料以 Google Sheet 同步為主。
