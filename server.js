...
function buildCaseId(type, row, source) {
  ...
  const canonical = canonicalizeRow(row);
  const explicit = normalizeText(canonical['案件編號']);
  if (explicit) return explicit;
  if (type === '告發') {
    const preferred = firstNonEmpty(canonical['稽查單號'], canonical['告發單號'], source['稽查單號'], source['告發單號']);
...
function caseFromFlatRow(type, row, options = {}) {
  ...
  const externalId = getCaseIdentifier(type, row);
  if (options.requireExplicitId && !externalId) {
    const err = new Error(type === '告發'
      ? 'Google Sheet 同步略過：告發案件未填稽查單號或告發單號'
      : 'Google Sheet 同步略過：通檢案件未填案件編號');
