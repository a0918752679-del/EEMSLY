  ...
  tbody.innerHTML = state.cases.map((item) => {
    ...
    return `
      <tr>
        ...
        <td><strong>${escapeHtml(item.id)}</strong><div class="case-meta">${escapeHtml(s['稽查單號'] || s['稽查編號'] || s['告發單號'] || '')}</div></td>
        ...
        <td>
          <div class="row-actions">
            ...
            <button class="secondary-btn small-btn edit-btn" data-id="${escapeHtml(item.id)}">修改</button>
            <button class="danger-btn small-btn reject-btn" data-id="${escapeHtml(item.id)}">退回</button>
            <button class="danger-btn small-btn delete-btn" data-id="${escapeHtml(item.id)}">刪除</button>
          </div>
        </td>
      </tr>
    `;
...
async function openEdit(id) {
  ...
  $('editOriginalId').value = item.id;
  $('editCaseIdInput').value = item.id;
...
async function syncPull() {
  if (!confirm('確認要從 Google Sheet 同步資料到系統？告發以稽查單號/告發單號、通檢以案件編號比對更新。')) return;
  const data = await api('/api/sync/pull', { method: 'POST' });
  alert(`同步完成：拉回 ${data.pulled} 筆，新增 ${data.inserted} 筆，更新 ${data.updated} 筆；缺少必要識別欄位略過 ${data.skippedMissingCaseId || 0} 筆。`);
  await loadCases();
}
