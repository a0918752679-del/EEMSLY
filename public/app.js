...
function editableSourceHeaders(type) {
  ...
  const priority = ['承辦複核','是否累犯','1年內再次違反','違規日期','前次告發','日數','車號','車種','量測日期','稽查時間','量測位置地點','便於複製(行政區)','便於複製(路段)','背景修正後分貝','管制標準','金額','稽查編號','告發單號','廠牌','出廠年月','車主姓名','證號','生日','郵遞區號','戶籍地','備註'];
...
async function syncPull() {
  ...
  alert(`同步完成：拉回 ${data.pulled} 筆，新增 ${data.inserted} 筆，更新 ${data.updated} 筆；未填案件編號略過 ${data.skippedMissingCaseId || 0} 筆。`);
  await loadCases();
}
